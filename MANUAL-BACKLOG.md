...
## 🟡 M6 — The landing page is built. Two decisions left, and both are yours

Backlog G2, and it is no longer a question of whether: **the site exists**, at
`C:\Dev\KeyholdLandingPage`. One page — hero, the audit trail, features, architecture,
screenshots, the honest trade-offs table, the download split, and an FAQ. React + Vite +
TypeScript strict, hand-written CSS over Keyhold's own Midnight and Dawn palettes, no Tailwind,
no third-party request of any kind. `npm install && npm run dev` to see it; `npm run verify` is
the whole gate and it is green.

Two things were deliberately **not** done, because both are yours to decide:

### 1. It is not a git repository

`git init` was not run, and nothing is committed. The rule in the global `CLAUDE.md` is that a
directory with no repository does not get one without you saying so, and creating a second
public-facing repository for this project is a decision rather than a chore.

**To make it one:**

```bash
cd C:\Dev\KeyholdLandingPage
git init -b main
git add .                        # .gitignore already covers node_modules and dist
git commit -m "The Keyhold landing page"
gh repo create AnahatM/KeyholdLandingPage --private --source=. --push
```

`gh` is at `C:\Program Files\GitHub CLI\gh.exe` and may not be on the PATH — see M1.

**Check before pushing**, exactly as M1 did: `public/screenshots/` holds captures of a _smoke
vault_, generated by `tools/smoke.mjs` against throwaway data in a temp directory. There is no
real credential in any of them, and the one visible path is a temp folder. Worth confirming by
eye anyway, because a screenshot is the easiest way to publish something by accident.

### 2. It is not deployed

Vercel and Cloudflare Pages are both fine and both free for this. It builds to plain static
files in `dist/`, so any host works. **Nothing was deployed and no account was touched** —
deploying is outward-facing and it is your call which platform this lives on.

Whichever you pick, the build command is `npm run build` and the output directory is `dist`.
If it goes to a custom domain, `index.html` has a `<link rel="canonical">` pointing at
`https://keyhold.app/` that needs updating to the real address — it is the only place a URL for
the site itself is written down.

**One thing to change after the repository is public:** the site's download card links to
`github.com/AnahatM/Keyhold/releases`, which 404s for anyone who is not you until M5 is done.

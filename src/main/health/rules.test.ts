 ...
 * before you trust it"). All four were caught, and all four were reverted:
 *
 ...
 *   | trashed filter replaced with `filter(() => true)` | 2 failed — the trashed-exclusion block and the trashed-joins-a-cluster case |
...
describe('trashed records are excluded from every rule', () => {
  ...
  it('reports nothing at all once the same records are in the trash', () => {
    const report = analyseVault(vault(maximallyBroken(true)), { now: NOW });
    for (const rule of HEALTH_RULE_IDS) {
...
describe('rules are independently toggleable', () => {
  it('drops exactly the disabled rule and nothing else', () => {
    ...
    const all = analyseVault(document, { now: NOW });

    for (const rule of HEALTH_RULE_IDS) {
      const without = analyseVault(document, { now: NOW, enabledRules: { [rule]: false } });
  ...
  it('never lowers the score by switching a rule off', () => {
    ...
    const document = vault(maximallyBroken(false));
    const all = analyseVault(document, { now: NOW }).score;
    for (const rule of HEALTH_RULE_IDS) {
      const without = analyseVault(document, { now: NOW, enabledRules: { [rule]: false } });
...
function vaultFullOfSecrets(): VaultDocument {
  // Every password contains the marker, several records share one (so clusters form), and
  // one URL carries credentials in its userinfo — the case that makes copying a raw URL
  // into the report a leak rather than a convenience.
  ...
  return vault([
    ...
    record({ id: 'c', password: marker('unique-c'), customValue: marker('custom-c') }),

...
| [`02-Decision-Log.md`](./02-Decision-Log.md)         | ADR-style record of every decision — D1-D26: D1-D17 are the founding set, D18-D22 came out of the Phase 0 scaffold, and D23-D26 were recorded after the fact for systems that shipped ahead of their entry — and the alternatives rejected, plus the decisions deliberately deferred to implementation. The range is guarded: `tools/doc-counts.test.ts` reads the last `### D<n>` heading in the log and fails if this row stops naming it. |

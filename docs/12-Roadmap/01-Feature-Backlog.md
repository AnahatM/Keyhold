...
| E23 | Component tests for the settings and organisation screens | ⭐⭐     | **Two complete in-memory gateway doubles exist and nothing imports them** — `settings/fake-gateway.ts` (174 lines) and `organisation/fake-gateway.ts` (253), found by `tools/renderer-is-reachable.test.ts` on its first run. The hard part of testing those two screens is already written; what is missing is the tests that drive it. Either write them or delete the doubles, but not neither |
| E24 | Fold away the unused import and export barrels            | ⭐       | `renderer/src/import/index.ts` and `export/index.ts` each document "one import site for whoever mounts this" and give the lines to do it — and both wizards are mounted by direct path, so the barrels are an unused second route whose instructions can drift from the real one. Hard rule 8. Either route through them or delete them                                                           |

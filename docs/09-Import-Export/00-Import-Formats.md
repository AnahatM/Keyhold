...
> The nineteen parsers, what each column becomes, and what is deliberately dropped.
...
> rotted once from eleven to twelve with nothing to catch it, and has caught every jump since.
...
| **1Password 1PUX**    | The `.1pux` archive, not the CSV: a ZIP holding `export.data`. Vaults→folders (account-prefixed when there is more than one) · `loginFields` by `designation` · `sections[].fields[]` typed from the JSON key, so a `concealed` value is a secret whatever its label says · `totp`→`otp-secret` · tags · `favIndex`→favourite · trashed items skipped, reported. **Attachments are counted and reported, never imported** — keep the `.1pux` until they are re-attached                                                                                                                                                                                                                                                                                   |
| **Proton Pass**       | `data.json` from the export zip. Vault name→folder · `itemEmail` and `itemUsername` kept apart, as Proton keeps them · `urls` · `totpUri` · `extraFields` with Proton's own type honoured (`hidden`→`password`, never re-guessed) · `pinned`→favourite · trashed items skipped, reported · passkeys→**dropped, reported** (bound to Proton's store; nobody can export them)                                                                                                                                                                                                                                                                                                                                                                               |
| **Enpass**            | `folders[]` nested through `parent_uuid` · every typed field mapped, with **`sensitive: 1` overriding a field's first-class home**: an explicitly-sensitive value never lands in `username`, `email` or `urls`, because those cross into the renderer · fields Enpass tombstoned with `deleted: 1` skipped, reported — importing one resurrects a password the user replaced                                                                                                                                                                                                                                                                                                                                                                              |
| **Dashlane JSON**     | The richer sibling of the CSV: a section map (`AUTHENTIFIANT`, `SECURENOTE`, `PAYMENTMEANS_CREDITCARD`, …), every value written as a string. Known sections mapped, **unknown sections imported anyway** as labelled custom fields; `secondaryLogin`→"Alternate login" with the same label the CSV parser uses, so both Dashlane paths produce the same record                                                                                                                                                                                                                                                                                                                                                                                            |
| **Keeper**            | Seven fixed columns then **positional** custom-field name/value pairs, which is why this one builds its own table rather than using a header mapping. Headed and headerless exports both accepted; a pair whose name is blank is forced to `password`, because a value nobody can label is likelier a secret than not                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **RoboForm**          | `Url` · `Name` · `MatchUrl` · `Login` · `Pwd` · `Note` · `Folder` (`/`-rooted). `MatchUrl` is **not** a second URL — it may be a wildcard, and a made-up address in the URL list would be used by every future domain match — so it is kept as a field, and only when it differs from `Url`                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **KeePass XML**       | KeePassXC's plain `.xml` export, which carries what the CSV drops: the full group tree→folders, every custom `String` under the label the user chose, and the TOTP settings. Read with `xml-reader.ts` (D31), not a dependency. **The database's own root group is not a folder** — its name is the file's, and letting it through would file the whole vault one level deep. An entry's `History` is not walked: those are previous versions of the same entry, and importing them would multiply the vault and resurrect every password the user had replaced. Recycle bin skipped, reported · `Times/Expires`→**reported, not mapped** (an expiry already past would flag the record in health on day one) · a still-protected value skipped, reported |
...
- **KDBX**, KeePass's encrypted database. **Version 4 is intended and unbuilt**; version 3 is
  **decided against** and will be refused by name — its inner values are protected with
  Salsa20, which Node does not provide, and hand-writing a stream cipher is what "never invent
  cryptography" forbids. KeePassXC re-saves a 3 as a 4 by default, which is the advice the
  refusal gives. See D32.
- Everything else once listed here **is built**, and the drift is recorded rather than quietly
  erased: 1PUX, Proton Pass, Enpass, Keeper, RoboForm, Dashlane JSON and KeePass XML are all
  in `PARSERS` with a row in §3. Keyhold's own JSON export is `keyhold-json.ts`, first in the
  registry and the one strict parser (see [`01-Export-Formats.md`](./01-Export-Formats.md)
  §7), and an encrypted `.keep`/`.keepx` is imported by
  `import-service/vault-source.ts` — it is decrypted and re-serialised into that same JSON
  rather than given a parser of its own, which is D30.
...
confidence. So is KeePass XML: its schema is element-named rather than positional, so a
column cannot silently shift, and the two things it could get wrong — the root group and
`History` — each have their own case with the injection recorded. Safari and Dashlane are
medium-high — the optional OTP columns vary by build,

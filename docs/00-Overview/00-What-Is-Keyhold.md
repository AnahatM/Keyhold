...
| **G5** | Answer *"what changed, when, and from where?"* | Per-credential, per-field version history with a device and network audit trail |
...
to know *which machine* changed a password and *when*; and anyone unwilling to pay a subscription

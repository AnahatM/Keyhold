...
3. **And that you can leave at any time** — imports from nineteen managers, a lossless JSON export and Bitwarden's own column set, and a format documented well enough for somebody else to write a reader. (KDBX 4 export is intended and not yet built.)

...
| **The vendor turns hostile, raises prices, or shuts down** | There is no vendor. GPL-3.0, no server, KDBX 4 export, and a published format spec. |
...
| **Traffic analysis of the opt-in HIBP check** | If enabled, a network observer learns that you checked *something*, and sees a 5-character hash prefix matching roughly 400–800 real passwords. | It is off by default. Leave it off, or run it once over a VPN. Never sends the password or the full hash. |
...
6. **Record metadata** — titles, usernames, URLs, tags. Encrypted in the file, but present in renderer memory while unlocked. A compromised renderer learns *which accounts you have*, but not the passwords. This is a real, accepted residual risk, recorded here rather than hidden.

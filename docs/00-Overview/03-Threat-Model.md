...
| **The vendor turns hostile, raises prices, or shuts down**                                                | There is no vendor. GPL-3.0, no server, a published format spec, and four export formats today — encrypted parcel, Keyhold JSON, Keyhold CSV and a Bitwarden-compatible CSV. KDBX 4 export is planned for Phase 11 and is **not** built; when it lands it will add no dependency (D32).                                                                                                                                                                  |
| **A path that dials out** — a `.lnk`, a `.url` or a dragged file naming `\\attacker.example\share\x.keep` | On Windows, calling `stat` on a UNC path opens an SMB connection to a host the attacker named and performs an NTLMv2 handshake with the logged-in user's credentials. Every path from outside is checked against an **allow-list of shapes that name local storage** — a drive letter plus a separator, or a single POSIX `/` — so a UNC path, a device path and a drive-less root are all refused before anything touches the filesystem. Decision D25. |
| **Silent data loss**                                                                                      | Atomic writes, rolling backups, tombstones, mandatory pre-merge backups, trash with restore, undo on destructive actions, dry-run imports, and a merge that **refuses** rather than resolving a document holding two entries under one id (D26).                                                                                                                                                                                                         |
...
| **Traffic analysis of the opt-in HIBP check**               | If enabled, a network observer learns that you checked _something_, and sees a 5-character hash prefix matching roughly 400–800 real passwords. | It is off by default, behind **two** switches — a machine-scoped kill-switch (D23) and the vault's own setting — and neither is on today. Leave them off, or run it once over a VPN. It never sends the password or the full hash. |
